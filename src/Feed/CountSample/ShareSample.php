    ...
    public function __construct($feedEntry){
        if(!is_null($feedEntry)){
            ...
            if($feedEntry->getShares() > 0 && $feedEntry->getType() === FEED_ENTRY_TYPE_POST){
                ...
                $stmt = $mysqli->prepare("SELECT u.* FROM `feed` AS f INNER JOIN `users` AS u ON f.user = u.id WHERE f.type = 'SHARE' AND f.post = ? ORDER BY f.time DESC LIMIT " . CountSample::SAMPLE_LIMIT);
