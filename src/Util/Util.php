				...
				if(count($mediaFiles) == 1){
					...
					foreach($mediaFiles as $mediaFile){
						if($mediaFile->getType() == "IMAGE"){
							...
							$s .= '</div>';*/
							$s .= '<div class="border border-primary' . $d . ' bg-dark ignoreParentClick" style="max-height: 500px; height: 100%; background-image: url(\'' . $mediaFile->getThumbnailURL() . '\'); background-size: cover; ' . (!is_null($postId) ? ' cursor: pointer;" onclick="showMediaModal(\'' . $mediaFile->getId() . '\',' . $postId . ');"' : "\"") . '></div>';
							$s .= '</div>';
