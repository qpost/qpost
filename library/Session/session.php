...
function openSession($path,$name){
	/*$s = mysqli_query($link,"INSERT INTO `gigadrive_sessions` (`id`,`data`,`lastAccessTime`,`ip`) VALUES('" . mysqli_real_escape_string($link,$name) . "','',CURRENT_TIMESTAMP,'" . mysqli_real_escape_string($link,getIP()) . "') ON DUPLICATE KEY UPDATE `lastAccessTime` = CURRENT_TIMESTAMP");
