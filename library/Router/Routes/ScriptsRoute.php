		...
		if(!is_null($post)){
			...
			if(!is_null($user)){
				...
				if($post->getReplies() > 0){
					...
					$stmt = $mysqli->prepare("SELECT f.*,u.`id` AS `userId`,u.`displayName`,u.`username`,u.`email`,u.`avatar`,u.`bio`,u.`token`,u.`privacy.level`,u.`featuredBox.title`,u.`featuredBox.content`,u.`lastGigadriveUpdate`,u.`time` AS `userTime` FROM `feed` AS f INNER JOIN `users` AS u ON f.user = u.id WHERE f.`post` = ? AND f.`type` = 'POST' ORDER BY u.`id` = ?,f.`time` DESC");
					...
					if($stmt->execute()){
						...
						if($result->num_rows){
							while($row = $result->fetch_assoc()){
								...
								$u = User::getUserByData($row["userId"],$row["displayName"],$row["username"],$row["email"],$row["avatar"],$row["bio"],$row["token"],$row["privacy.level"],$row["featuredBox.title"],$row["featuredBox.content"],$row["lastGigadriveUpdate"],$row["userTime"]);
