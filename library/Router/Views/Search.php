...
<?php
	...
	if($num > 0){
		...
		if($type == "posts"){
			$stmt = $mysqli->prepare("SELECT p.`id` AS `postID`,p.`text` AS `postText`,p.`time` AS `postTime`,u.* FROM `feed` AS p INNER JOIN `users` AS u ON p.user = u.id WHERE p.`text` LIKE ? OR u.`displayName` LIKE ? OR u.`username` LIKE ? ORDER BY p.`time` DESC LIMIT " . (($page-1)*$itemsPerPage) . " , " . $itemsPerPage);
			$stmt->bind_param("sss",$q,$q,$q);
			if($stmt->execute()){
				$result = $stmt->get_result();
