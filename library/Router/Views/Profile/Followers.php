	...
	if($stmt->execute()){
		...
		if($result->num_rows){
			while($row = $result->fetch_assoc()){
				array_push($users,User::getUserByData($row["id"],$row["displayName"],$row["username"],$row["email"],$row["avatar"],$row["bio"],$row["token"],$row["privacy.level"],$row["featuredBox.title"],$row["featuredBox.content"],$row["time"]));
			}
		}
	}
	$stmt->close();
