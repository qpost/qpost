...
if($num > 0){
	...
	$stmt = $mysqli->prepare("SELECT * FROM `feed` WHERE ((`post` IS NULL AND `type` = 'POST') OR (`type` = 'SHARE')) AND `user` = ? ORDER BY `time` DESC LIMIT " . (($currentPage-1)*$itemsPerPage) . " , " . $itemsPerPage);
